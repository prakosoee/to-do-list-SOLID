<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6">
            <form method="GET" action="{{ route('task.index') }}" class="w-full md:w-1/3 mb-4 md:mb-0 flex gap-2">
            ...
            <table class="w-full table-auto">
                ...
                <tbody class="text-gray-600 text-sm dark:text-gray-300">
                    <tr class="border-b border-gray-200 hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-800">
                        <td class="py-3 px-6 text-left">{{ ($tasks->currentPage() - 1) * $tasks->perPage() + $loop->iteration }}</td>
                        <td class="py-3 px-6 text-left">{{ $t->title }}</td>
                        <td class="py-3 px-6 text-left">{{ $t->description }}</td>
                        <td class="py-3 px-6 text-left">
                            <span class="inline-block px-3 py-1 text-white rounded text-sm font-medium
                                @if($t->status == 'pending') bg-red-500
                                @elseif($t->status == 'in_progress') bg-yellow-500
                                @elseif($t->status == 'completed') bg-green-500
                                @endif">
                                {{ ucfirst(str_replace('_', ' ', $t->status)) }}
                            </span>
                        </td>
                        <td class="py-3 px-6 text-left">{{ $t->due_date }}</td>
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center dark:hover:text-gray-300">
                                ...
                                <!-- Tombol Delete -->
                                <form id="deleteTaskForm-{{ $t->id }}" action="{{ route('task.destroy', $t->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <!-- Tombol Delete -->
                                    <button type="button" data-task-id="{{ $t->id }}" onclick="openModal(this)"
        ...
        <div class="mt-4 flex justify-between items-center">
            ...
            @else
                <a href="{{ $tasks->previousPageUrl() }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
                    Previous
                </a>
            @endif
            ...
            @if ($tasks->hasMorePages())
                <a href="{{ $tasks->nextPageUrl() }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
                    Next
                </a>
            @else
                <span class="px-4 py-2 bg-gray-200 text-gray-400 rounded cursor-not-allowed">
